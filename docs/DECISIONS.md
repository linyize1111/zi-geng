...
## 2026-07-23 — 一次 bulk 多來源灌庫；週更改增量；品質門檻

- **發現**：使用者要資料庫一次更豐富、多網站爬，但仍要更新；且不要半日／蒙衝級廢詞。
- **採用**：`bulk`／`update` 雙模式；來源擴至維基詞典／語錄／文庫／百科＋教育部三典；`vocab-quality` 上傳前篩選；purge 下架低價值詞。
- **未採用**：停掉週更；爬商業字典。

---

- **發現**：詞卡單調；技巧僅 3 張；詞卡詳情欄位在 DB 卻未顯示。
- **採用**：主題詞庫種子；技巧約 24 張；今日／學習顯示類別、例句、強弱例；抽卡偏主題詞並做類別分散；小說本機 JSON 備份。
- **未採用**：等待使用者大綱再動小說情節內容。

---

- **發現**：題目偏場景小品；小說任務缺系統；使用者大綱稍後才給。
- **採用**：擴充題庫（日記→哲學等多主題）；小說任務改七階段計畫題庫；本機專案加 `plan` 與大綱保留區。
- **未採用**：在未收到大綱前虛構情節；把小說全文編輯一次做完。
- **證據**：使用者要求題目多樣、小說以現構思建系統並保留大綱區。

---

- **發現**：使用者認為庫太單調、幾乎都是國中程度成語；要衣料對照（紗／錦）、濫觴、生氣語域等大學生～職業作家詞彙；名言勿堆自創箴言。
- **採用**：寫作者詞庫 + 教育部《重編國語辭典修訂本》文學子集 + 成語上限約 250；名言改公有領域經典句；下架 `author_name=字耕` 自創箴言；今日抽卡約 75% 優先非成語。
- **未採用**：只擴充成語筆數；繼續把「字耕」箴言當主力名言。
- **證據**：使用者明確點名類型與程度區間。
